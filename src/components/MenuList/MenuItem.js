...
const MenuItem = ({ data: { menu, image, price, recommended } }) => {
    return (
        <div class={`max-w-md w-full flex border-b border-grey-light ${image === "" ? ("h-16") : ("")} md:my-2 mx-auto md:rounded-r md:shadow-md md:border-0`}>
            {image && <div class="m-1 h-24 w-1/4 flex-none bg-cover rounded text-center overflow-hidden" style={{ backgroundImage: `url('${image}')` }}></div>}
